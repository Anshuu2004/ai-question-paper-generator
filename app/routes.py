...
def generate_paper():
    ...
    try:
        ...
        test_response = test_model.generate_content("Say 'Hello, World!' in one word.")
...
"""
        ...
        try:
            current_app.logger.info("Attempting to create GenerativeModel with 'gemini-1.5-flash'")
            GenerativeModel = getattr(genai, "GenerativeModel")
            model = GenerativeModel("gemini-1.5-flash")  # Using the basic model name
            ...
            current_app.logger.error(f"AI model error with gemini-1.5-flash: {model_error}")
            # Try a fallback model
            try:
                current_app.logger.info("Attempting fallback with 'gemini-1.5-pro'")
                GenerativeModel = getattr(genai, "GenerativeModel")
                model = GenerativeModel("gemini-1.5-pro")  # Fallback to gemini-1.5-pro
                response = model.generate_content(prompt)
                raw_text = response.text.strip()
                current_app.logger.info("Fallback response received successfully")
            except Exception as fallback_error:
                current_app.logger.error(f"AI model error with fallback gemini-1.5-pro: {fallback_error}")
                # Try another fallback model
                try:
                    current_app.logger.info("Attempting fallback with 'gemini-pro'")
                    GenerativeModel = getattr(genai, "GenerativeModel")
                    model = GenerativeModel("gemini-pro")  # Another fallback option
                    response = model.generate_content(prompt)
                    raw_text = response.text.strip()
                    current_app.logger.info("Second fallback response received successfully")
                except Exception as second_fallback_error:
                    current_app.logger.error(f"AI model error with second fallback gemini-pro: {second_fallback_error}")
    ...
    <head>
        ...
        <style>
            ...
            * {
            ...
            .paper-header h1 { font-size: 20pt; margin: 0; }
            .paper-header h2 { font-size: 16pt; margin: 5px 0; font-weight: normal; }
            .paper-header h3 { font-size: 14pt; margin: 5px 0; font-weight: normal; }
            .details { 
                display: flex; 
                    ...
                    <ol class="question-list">
                        ...
                        <li class="question">
                            ...
                            <ol class="options">
                                ...
                                <li class="option">({{ loop.index0 | to_char }}) {{ opt }}</li>
                                {% endfor %}
                            </ol>
                            {% endif %}
                        </li>
